...
#include <ArduinoOTA.h>        // 🔄 Bibliothèque pour la mise à jour OTA (Over-The-Air) depuis l'IDE Arduino
...
// 🔴 Repere 2
// Structure unique associant, pour un même canal : le relais (pin + nom affiché)
// ET le bouton poussoir qui le pilote, ainsi que ses variables d'anti-rebond dans CANAUX[]
...
// valeurs fixes sur la carte ESP32 4 Relais  K1 GPIO32 - K2 GPIO33 - K3 GPIO25 - K4 GPIO26
struct Canal {
  byte pinRelais;                  // GPIO du relais
  const char* nom;                 // Nom affiché (OLED + page HTML)
  byte pinBP;                      // GPIO du bouton poussoir associé
  bool etatStable;                 // Dernier état stable retenu (anti-rebond)
  bool derniereLecture;            // Dernière lecture brute de la broche du BP
  unsigned long dernierChangement; // Horodatage du dernier changement détecté (anti-rebond)
};

// Pilotage des 4 relais + leur BP associé : BP14->K1(32), BP16->K2(33), BP17->K3(25), BP18->K4(26)
//Pour ajouter un 5ème canal, il suffit d'ajouter une ligne dans CANAUX[] (GPIO relais, Nom Bouton, GPIO BP, HIGH, HIGH, 0)
Canal CANAUX[] = {
  {32, "Relais K1", 14, HIGH, HIGH, 0},   // Relais K1 GPIO32 <- BP GPIO14
  {33, "Relais K2", 16, HIGH, HIGH, 0},   // Relais K2 GPIO33 <- BP GPIO16
  {25, "Relais K3", 17, HIGH, HIGH, 0},   // Relais K3 GPIO25 <- BP GPIO17
  {26, "Relais K4", 18, HIGH, HIGH, 0}    // Relais K4 GPIO26 <- BP GPIO18
};

// Calcule automatiquement le nombre de canaux à partir de CANAUX[]
const byte NB_CANAUX = sizeof(CANAUX) / sizeof(CANAUX[0]);
...
void genererTableauJSPins(WiFiClient &client);// génère la ligne JS "let pins=[...]" à partir de CANAUX[]
...
// --- 🔄 État de la mise à jour OTA (Over-The-Air), utilisé pour l'affichage OLED ---
// L'affichage OLED pendant l'OTA (démarrage/progression/fin) est géré directement
// dans les callbacks ArduinoOTA définis dans configurerOTA(), comme dans le
// programme "Programmateur_horaire_ESP32_multi_relais_Web".
void configurerOTA();                         // 🔄 Configure les callbacks ArduinoOTA (à appeler une fois le WiFi connecté)
bool otaEnCours = false;        // Devient vrai dès qu'un transfert OTA démarre (bloque l'affichage normal de l'OLED)
unsigned int otaProgression = 0; // Pourcentage de progression du transfert OTA en cours (0-100)
...
// 🔴 Repere 3
  // Plus besoin de vérifier la correspondance PINS[]/NOMS[] : la structure CANAUX[]
  // garantit qu'une pin et son nom sont toujours définis ensemble, sur la même ligne.

  // 🔴 Repere 3b - Initialise en une seule boucle chaque canal : le relais (sortie) et son BP (entrée)
  for (byte i = 0; i < NB_CANAUX; i++)      // Parcourt chaque canal déclaré dans CANAUX[]
  {
    pinMode(CANAUX[i].pinRelais, OUTPUT);         // Déclare la broche du relais comme une sortie numérique
    digitalWrite(CANAUX[i].pinRelais, LOW);       // Force la sortie courante à l'état bas (éteinte) au démarrage

 // pinMode(CANAUX[i].pinBP, INPUT_PULLDOWN); // repos = LOW, appui = HIGH (via +3.3V) voir modif a faire dans void gererBoutons()
    pinMode(CANAUX[i].pinBP, INPUT_PULLUP);   // repos = HIGH, appui = LOW (Via GND) la broche du BP en entrée avec résistance de tirage interne
  }
  ...
  if (WiFi.status() == WL_CONNECTED)
{
   ...
   Serial.println(F("\nServeur initialisé, attente de l'adresse IP..."));
    ...
    configurerOTA();      // 🔄 Active la mise à jour OTA (Wifi > Port de l'IDE Arduino) avec retour sur l'OLED
}
}

void loop() {

  ArduinoOTA.handle();   // 🔄 Doit être appelé le plus souvent possible pour ne rater aucun paquet de mise à jour

  // 🔄 Pendant un transfert OTA, on n'exécute rien d'autre (pas de serveur web, pas de boutons)
  // afin de laisser toute la ressource CPU/flash à l'écriture du nouveau programme.
  if (otaEnCours) return;
  ...
  {
    ArduinoOTA.handle();   // 🔄 Garde l'OTA réactif même si un client web reste connecté pendant ce temps
...
void gererBoutons() {
  for (byte i = 0; i < NB_CANAUX; i++)
  {
    bool lecture = digitalRead(CANAUX[i].pinBP);   // Lecture brute de l'état actuel de la broche du BP

    if (lecture != CANAUX[i].derniereLecture)      // La lecture vient de changer par rapport au tour précédent
    {
      CANAUX[i].dernierChangement = millis();      // On redémarre le chrono anti-rebond
    }

    if ((millis() - CANAUX[i].dernierChangement) > DEBOUNCE_BP_MS)  // Le signal est stable depuis assez longtemps
    {
      if (lecture != CANAUX[i].etatStable)         // L'état stable a réellement changé (et non un simple rebond)
      {
        CANAUX[i].etatStable = lecture;            // Mémorise le nouvel état stable
      ...
      // plutôt que vers la masse), il faudra inverser la logique LOW/HIGH.
        {
          bool etatActuelRelais = digitalRead(CANAUX[i].pinRelais);
          digitalWrite(CANAUX[i].pinRelais, !etatActuelRelais);  // Bascule (toggle) le relais associé
        }
      }
    }

    CANAUX[i].derniereLecture = lecture;           // Mémorise la lecture brute pour la comparaison du prochain tour
  }
}
...
void traiterCommande(const String &req) {         // Reçoit la ligne HTTP par référence constante pour éviter de la dupliquer
  ...
  {
    for (byte i = 0; i < NB_CANAUX; i++)      // Teste chaque sortie du tableau, une par une
    {
      // Création d'un mini-buffer de caractères pour générer les requêtes à tester (ex: "/P5ON")
      char cmdOn[12];
      char cmdOff[12];
      sprintf(cmdOn, "/P%dON", CANAUX[i].pinRelais);
      sprintf(cmdOff, "/P%dOFF", CANAUX[i].pinRelais);

      if (req.indexOf(cmdOn) >= 0)      // Si la ligne demande d'allumer cette sortie
      {
        digitalWrite(CANAUX[i].pinRelais, HIGH);           // Met la broche à l'état haut (allumée)
      ...
      {
        digitalWrite(CANAUX[i].pinRelais, LOW);            // Met la broche à l'état bas (éteinte)
  ...
  else if (req.startsWith("GET /ALL_ON"))        // Si la ligne demande de TOUT allumer
  {
    for (byte i = 0; i < NB_CANAUX; i++)
    {
      digitalWrite(CANAUX[i].pinRelais, HIGH);         // Force toutes les broches à l'état haut
    }
  }
  else if (req.startsWith("GET /ALL_OFF"))       // Si la ligne demande de TOUT éteindre
  {
    for (byte i = 0; i < NB_CANAUX; i++)
    {
      digitalWrite(CANAUX[i].pinRelais, LOW);          // Force toutes les broches à l'état bas
    }
  }
}
...
void genererTableauJSPins(WiFiClient &client) {
  ...
  for (byte i = 0; i < NB_CANAUX; i++)        // Parcourt chaque broche déclarée dans CANAUX[]
  {
    client.print(CANAUX[i].pinRelais);               // Écrit le numéro de broche courant (ex: 5)
    if (i < NB_CANAUX - 1) client.print(","); // Ajoute une virgule séparatrice sauf après le dernier élément
  }
...
String genererCasesHTML() {
  ...
  html.reserve(NB_CANAUX * 220UL);     // Réserve à l'avance ~220 caractères par case pour éviter les réallocations

  for (byte i = 0; i < NB_CANAUX; i++)     // Parcourt chaque sortie pour générer sa "case" HTML
  {
    byte pin = CANAUX[i].pinRelais;              // Récupère le numéro de broche de la sortie courante
    ...
    html += "<div class='titre'>"; html += CANAUX[i].nom; html += "</div>"; // affiche le nom personnalisé au lieu de "Sortie Dx"
...
function majAffichage(rawText){
  ...
  // à partir de CANAUX[] (voir fonction genererTableauJSPins côté C++).
  // Ne pas modifier cette ligne à la main : elle est réécrite à chaque
  // chargement de page en fonction du contenu réel de CANAUX[].
)====="));

  // --- PARTIE DYNAMIQUE : génère ici la ligne "let pins=[5,6,7,8];"
  // en fonction du contenu réel de CANAUX[], au lieu d'un tableau figé.
...
void envoyerEtat(WiFiClient &client)  {   // Envoie une réponse courte contenant seulement l'état des sorties
  ...
  for (byte i = 0; i < NB_CANAUX; i++) 
  {
    client.print(digitalRead(CANAUX[i].pinRelais));
    if (i < NB_CANAUX - 1) client.print(";");
  }
...
// 🔄 Modes OTA
//============================================================
// configurerOTA : met en place les callbacks ArduinoOTA (début, progression,
// fin, erreur) afin de suivre l'état de la mise à jour sur le moniteur série
// ET sur l'écran OLED. À appeler une seule fois, dans setup(), après que le
// WiFi soit connecté.
//============================================================
// L'affichage OLED pendant l'OTA (démarrage / progression / fin) reprend ici
// exactement le même contenu et la même mise en page que le programme
// "Programmateur_horaire_ESP32_multi_relais_Web" (fonction setupOTA()),
// pour que les deux programmes se comportent de façon identique à l'écran.
void configurerOTA() {

  ArduinoOTA.setHostname(HOSTNAME_MDNS);   // Le module apparaîtra sous ce nom dans le menu "Ports réseau" de l'IDE Arduino
  ...
  ArduinoOTA.onStart([]() {
    // ⚠️ Pendant l'OTA, on coupe les relais par sécurité (évite qu'ils restent bloqués sur un état imprévu)
    for (byte i = 0; i < NB_CANAUX; i++) {
      digitalWrite(CANAUX[i].pinRelais, LOW);
    }
  ...
  ArduinoOTA.onProgress([](unsigned int progress, unsigned int total) {
    otaProgression = (total > 0) ? (progress * 100 / total) : 0;
    Serial.printf("Progression OTA : %u%%\r", otaProgression);

    display.clearDisplay();
    display.setCursor(12, 16);  // Décale de 2 caractères vers la droite et 2 lignes vers le bas
    display.printf("Mise a jour : %d%%", otaProgression);
    display.drawRect(12, 32, 100, 10, 1);
    // Dessine la barre de progression. La largeur du rectangle plein va de 0 à 100 pixels
    display.fillRect(12, 32, otaProgression, 10, 1);
    display.display();
  });
...
void afficherEcranOLED() {
  ...
  for (byte i = 0; i < NB_CANAUX; i++)
  {
    bool etat = digitalRead(CANAUX[i].pinRelais);
    display.setCursor(0, y);
    display.print(CANAUX[i].nom);         // Nom du relais (ex: "Salon")
    display.print(F(": "));
